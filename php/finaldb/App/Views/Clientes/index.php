...
<h1 class="text-center p-3">Panel de Clientes</h1>
<div class="container-fluid row">
    <!-- Formulario -->
    <form class="col-4 p-3" method="POST" action="index.php?controller=ClienteController&action=registrarCliente">
    ...
    <div class="col-8 p-4">
        <form method="GET" class="mb-3 d-flex">
            <input type="hidden" name="controller" value="ClienteController">
        ...
        <table class="table table-striped">
            <thead class="bg-info text-white" id="clientesTable">
                <tr>
                    <th>ID</th>
                    <th>Categoría</th>
                    <th>Nombre</th>
            ...
            <tbody>
                <?php if (!empty($clientes)): ?>
                    <?php foreach ($clientes as $cliente): ?>
                        <tr>
                            <td><?= htmlspecialchars($cliente['clie_id']) ?></td>
                            <td><?= htmlspecialchars($cliente['clie_cod']) ?></td>
                            <td><?= htmlspecialchars($cliente['clie_nom']) ?></td>
                            <td><?= htmlspecialchars($cliente['clie_seg']) ?></td>
                            <td>
                                <a href="index.php?controller=ClienteController&action=cambiarEstadoCliente&id=<?= $cliente['clie_id'] ?>&estado=<?= $cliente['clie_activo'] ?>"
                                    class="btn btn-sm <?= $cliente['clie_activo'] ? 'btn-danger' : 'btn-success' ?>">
                                    <?= $cliente['clie_activo'] ? 'Desactivar' : 'Activar' ?> <i class="fa-solid fa-trash"></i>

                                    <a href="index.php?controller=ClienteController&action=viewCliente&id=<?= $cliente['clie_id'] ?>" class="btn btn-sm btn-warning">
